...
const AIChatSidebar: React.FC<AIChatSidebarProps> = ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (prompt.trim() !== '') {
            const context = useSelectedTextAsContext && selectedText ? selectedText : writingContent;
            setIsLoading(true);
            await onSendPrompt(prompt, context); // Await parent handler
            setIsLoading(false);
            setPrompt('');
        }
    };

    const truncateContext = (text: string | null, front = 15, back = 15) => {
        if (!text) return '';
    ...
    const contextDisplay = useSelectedTextAsContext ? selectedText : writingContent;
