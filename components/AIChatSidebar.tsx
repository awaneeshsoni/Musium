...
interface AIChatSidebarProps {
  onSendPrompt: (prompt: string, context: string | null) => Promise<void>;
  chatHistory: { role: string; content: string }[];
  selectedText: string | null;
  writingContent: string | null;
  useSelectedTextAsContext: boolean;
  onUseSelectedTextContext: () => void;
  currentContext: string | null; // NEW PROP
}
...
const AIChatSidebar: React.FC<AIChatSidebarProps> = ({
  ...
  currentContext, // Destructure new prop
}) => {
  const [prompt, setPrompt] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const chatContainerRef = useRef<HTMLDivElement>(null);
  // const [currentContext, setCurrentContext] = useState<string | null>(writingContent); // Removed local state

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (prompt.trim() !== '') {
      setIsLoading(true);
      await onSendPrompt(prompt, currentContext); // Use the currentContext from props
      setIsLoading(false);
      setPrompt('');
    }
  };

  const truncateContext = useCallback((text: string | null, front = 15, back = 15) => {
    if (!text) return '';
    const words = text?.split(' ') || []; // Use optional chaining here
  ...
  // useEffect(() => {
  //   // Update currentContext whenever selectedText or writingContent changes
  //   if (useSelectedTextAsContext && selectedText) {
  //     setCurrentContext(selectedText);
  //   } else {
  //     setCurrentContext(writingContent);
  //   }
  // }, [selectedText, writingContent, useSelectedTextAsContext]);

  const contextDisplay = currentContext; // Display currentContext
